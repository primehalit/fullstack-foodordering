  ...
  return (
    <div
      className={`h-[5.5rem] z-50 relative w-full ${
        router.asPath === "/" ? "bg-transparent" : "bg-white !fixed"
      }`}
    >
      <div className="container mx-auto text-black flex justify-between items-center h-full">
        ...
        sm:w-auto sm:h-auto w-full h-screen sm:text-white text-black
        sm:bg-transparent bg-white sm:flex hidden z-50 ${
          isMenuModal === true && "!grid place-content-center"
        }`}
        >
          <ul className="flex gap-x-2 sm:flex-row flex-col items-center text-black">
          <li
            ...
            <li
              ...
              onClick={() => setIsMenuModal(false)}
            >
              <Link href="/reservation">Reservasyon</Link>
            </li>
          </ul>
          {isMenuModal && (
            <button
        ...
        <div className="flex gap-x-4 items-center">
          ...
          >
            <FaBurger className="text-xl hover:text-primary transition-all" />
          </button>
        </div>
      </div>
